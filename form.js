let confirmVisit = () => {
	...
	if (nameInput == false || visitorNumber == false || availableDate == false) {
		incompleteForm.textContent = `Sorry, we need more information to continue.`;
	} else {
		form.remove();
		submissionMsg.textContent = `Great news, ${nameInput}! We can absolutely accomodate you and your ${visitorNumber} guest(s). Expect a call by ${availableDate}!`;
	}
};
